import time, socket, sys
import re,uuid
import math
from tkinter import *
from tkinter import messagebox
from functools import partial


mac=(':'.join(re.findall('..','%012x' %uuid.getnode())))
#mac='b8:ca:3a:d5:eb:53'

#8002281480558766684

def mac_to_int(mac):
    res = re.match('^((?:(?:[0-9a-f]{2}):){5}[0-9a-f]{2})$', mac.lower())
    if res is None:
        raise ValueError('invalid mac address')
    return int(res.group(0).replace(':', ''), 16)

a=mac_to_int(mac)
#print(a)


def serial():
    ser=""
    for x in mac:
        if x.isdigit():
            q=collatz(x)
            ser+=q
        elif x=="a":
            q=collatz(10)
            ser+=q
        elif x=="b":
            q=collatz(11)
            ser+=q
        elif x=="c":
            q=collatz(12)
            ser+=q
        elif x=="d":
            q=collatz(13)
            ser+=q
        elif x=="e":
            q=collatz(14)
            ser+=q
        elif x=="f":
            q=collatz(15)
            ser+=q
        
    return ser


def collatz(n):
    ser=""
    ser+=str(n)
    while int(n)>1:
        if n==16:
                break
        else:
            if (int(n) % 2):
                # n is odd
                n = 3*int(n) + 1
                ser+=str(n)
            else:
                # n is even
                n = int(int(n)/2)
                ser+=str(n)
    return ser

def GenerateKey(key):
    n1=83838793906163188026
    #alist=[]
    #blist=[]
    #k=''
    #j=0
    #for i in str(key):
        #alist.append(i)
    #while j<(len(alist)-1):
        #blist.append(abs(int(alist[j])-int(alist[j+1])))
        #j+=2
    #for i in blist:
        #k=k+str(i)
    #q=int(math.sqrt(int(k)))
    pk = pow(int(key), e, n1)
    return pk


n =     # p*q = modulus
e = 65537
d =  
#message="703084636222302833"
#print('message                 ', message)
 
#hex_data   = binascii.hexlify(message.encode())
#print('hex data                ', hex_data)

z=int(serial())
#print("\n\nserial key: ",z)
plain_text =z
#int(hex_data, 16)
#print('plain text integer      ', plain_text)
 
if plain_text > n:
  raise Exception('plain text too large for key')
 
encrypted_text = pow(plain_text, e, n)
#print('\nencrypted text integer  ', encrypted_text)

##########################################################################

print('Client Server...')
time.sleep(1)
#Get the hostname, IP Address from socket and set Port
soc = socket.socket()
shost = socket.gethostname()
ip = socket.gethostbyname(shost)
#get information to connect with the server
print(shost, '({})'.format(ip))
server_host = ip
print("server\'s IP address:",ip)
name = "Client"
port = 1234
print('Trying to connect to the server: {}, ({})'.format(server_host, port))
time.sleep(1)
soc.connect((server_host, port))
print("Connected...\n")
soc.send(name.encode())
server_name = soc.recv(1024)
server_name = server_name.decode()
print('{} has joined...'.format(server_name))
#while True:
   #message = soc.recv(1024)
   #message = message.decode()
   #print(server_name, ">", message)

#email=input('Enter your email address: ')
def verifyLogin():
        z=int(serial())
        k2=GenerateKey(z);
        print(key.get())
        print(k2)
        if k2==key.get():
            print("Product activated")
            messagebox.showinfo("Status","Product activated. Thank you "+username.get()+" for purchasing our product") 
        else:
            print("Invalid product key")
            messagebox.showinfo("Status","Invalid Product Key") 

def validateLogin():
    print(email.get())
    soc.send(email.get().encode())
    soc.send(message.encode())
    tkWindow = Toplevel()  
    tkWindow.geometry('400x400')  
    tkWindow.title('Product purchase page')
    keyLabel = Label(tkWindow, text="Product key").grid(row=0, column=0)
    
    keyEntry = Entry(tkWindow, textvariable=key ).grid(row=0, column=1)
    verifyButton = Button(tkWindow, text="Enter", command=verifyLogin).grid(row=4, column=0)
tkWindow = Tk()  
tkWindow.geometry('400x400')  
tkWindow.title('Product purchase page')
key = IntVar()
cnum=""
usernameLabel = Label(tkWindow, text="Name").grid(row=0, column=0)
username=StringVar();
usernameEntry = Entry(tkWindow, textvariable=username).grid(row=0, column=1)

emailLabel = Label(tkWindow, text="Email address").grid(row=1, column=0)
email = StringVar()
emailEntry = Entry(tkWindow, textvariable=email).grid(row=1, column=1)

creditLabel = Label(tkWindow, text="Credit card number").grid(row=2, column=0)
creditEntry = Entry(tkWindow, textvariable=cnum).grid(row=2, column=1)

loginButton = Button(tkWindow, text="Enter", command=validateLogin).grid(row=4, column=0)



message = str(encrypted_text)
   #if message == "[bye]":
      #message = "Leaving the Chat room"

print("\n")
      #break
#soc.send(message.encode())
print('\nThe product key has been sent to your email')
z=int(serial())

#i=int(input("Please enter the product key: "))

tkWindow.mainloop()

